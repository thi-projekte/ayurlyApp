...
const TopLikedChart = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const result = await apiRequest(`/api/admin/dashboard/performance/top-liked?${params}`, 'GET', null, keycloakInstance.token);
            const chartData = [['Inhalt', 'Likes', { role: 'annotation' }]];
            result.forEach(item => {
                chartData.push([`${item.title} (${item.contentType.substring(0,1)})`, item.count, item.count]);
            });
            setData(chartData);
        } catch (error) {
            console.error("Fehler beim Laden der Top-Liked-Daten:", error);
        } finally {
            setIsLoading(false);
        }
    ...
    const chartOptions = {
        hAxis: { title: 'Anzahl Likes', minValue: 0 },
        vAxis: { title: 'Inhalt' },
        bars: 'horizontal',
